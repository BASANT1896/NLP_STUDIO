...
def _nllb_model():
    def load():
        ...
        try:
            m = AutoModelForSeq2SeqLM.from_pretrained(NLLB_MODEL, dtype=torch.bfloat16)
        except TypeError:  # older transformers
            m = AutoModelForSeq2SeqLM.from_pretrained(NLLB_MODEL, torch_dtype=torch.bfloat16)
        return m.eval()

    return model_store.get_model("nllb", load)

...
def _load_gliner_lean():
    ...
    if "low_cpu_mem_usage" in inspect.signature(BaseGLiNER.from_pretrained).parameters:
        gl = GLiNER.from_pretrained(NER_MODEL, low_cpu_mem_usage=True)
        if shrink:
    ...
    try:
        gl = GLiNER.from_pretrained(NER_MODEL)
    finally:
    ...
    model_dir = Path(snapshot_download(NER_MODEL, local_files_only=True))

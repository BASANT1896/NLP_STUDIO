...
# ---------------------------------------------------------------- sentiment (local)
QUANTIZE_SENTIMENT = os.getenv("SENTIMENT_QUANTIZE", "1") != "0"
...
def _sentiment_pipe(model_id: str):
    def load():
        ...
        if QUANTIZE_SENTIMENT:
            try:  # in-place, so no second copy of the model is created
                model = torch.ao.quantization.quantize_dynamic(
                    model, {torch.nn.Linear}, dtype=torch.qint8, inplace=True)
            except Exception:  # noqa: BLE001  (fall back to the normal model)
                pass
        return pipeline("text-classification", model=model, tokenizer=tok,
                        top_k=None, truncation=True, max_length=512)

    return model_store.get_model(f"sentiment:{model_id}", load)
...
def _shrink_embeddings(model) -> None:
    """Store the giant word-embedding table in bfloat16 (outputs are converted back to float32)."""
    import torch

    class _BF16Embedding(torch.nn.Embedding):
        def forward(self, ids):
            return torch.nn.functional.embedding(ids, self.weight, self.padding_idx).float()

    for m in model.modules():
        if isinstance(m, torch.nn.Embedding) and m.num_embeddings >= 100_000:
            m.weight.data = m.weight.data.to(torch.bfloat16)
            m.__class__ = _BF16Embedding

def _gliner_dir() -> str:
    """Download only the safetensors weights (the repo also holds a duplicate 1.16 GB .bin file)."""
    from pathlib import Path

    from huggingface_hub import snapshot_download

    d = Path(snapshot_download(NER_MODEL, allow_patterns=["*.json", "*.safetensors"]))
    if not (d / "model.safetensors").exists():  # older revision without safetensors: get everything
        d = Path(snapshot_download(NER_MODEL))
    return str(d)


def _load_gliner_lean():
    """Load GLiNER without holding two full copies of its weights in RAM (works on old and new GLiNER)."""
    import inspect
    from pathlib import Path
    ...
    from huggingface_hub import snapshot_download
    model_dir = _gliner_dir()

    shrink = os.getenv("GLINER_BF16_EMBED", "1") != "0"

    # Newer GLiNER: built-in low-memory loading (meta-device model, weights assigned: one copy only).
    if "low_cpu_mem_usage" in inspect.signature(BaseGLiNER.from_pretrained).parameters:
        gl = GLiNER.from_pretrained(model_dir, low_cpu_mem_usage=True)
        if shrink:
            _shrink_embeddings(gl.model)
        model_store._release_memory()
        gl.eval()
        return gl

    # Older GLiNER (e.g. 0.2.24): build the model, skip its own loader, stream the weights in ourselves.
    real = BaseGLiNER.__dict__["_load_state_dict"]
    BaseGLiNER._load_state_dict = classmethod(lambda cls, *args, **kwargs: {})
    try:
        gl = GLiNER.from_pretrained(model_dir)
    finally:
        BaseGLiNER._load_state_dict = real
    ...
    with torch.no_grad():
        ...
        if st_file.exists():
            ...
            w = torch.load(model_dir / "pytorch_model.bin", map_location="cpu", mmap=True, weights_only=True)
            for k, v in w.items():
                if k in sd:
                    sd[k].copy_(v)
                    loaded.add(k)
    ...
    missing = [k for k in sd if k not in loaded and "position_ids" not in k]
    if missing:  # never silently run with random weights
        raise RuntimeError(f"{len(missing)} GLiNER weights were not found, e.g. {missing[:3]}")
    gl.eval()
    return gl

def _gliner():
    def load():
        print(f"[mem] peak so far: {model_store.peak_mb()} MB", flush=True)
        print(f"[mem] before GLiNER load: {model_store.rss_mb()} MB", flush=True)
        try:
            m = _load_gliner_lean()
        except Exception as e:  # noqa: BLE001  (fall back to the stock loader)
            print(f"[mem] lean GLiNER load failed ({e}); using the standard loader", flush=True)
            from gliner import GLiNER
            m = GLiNER.from_pretrained(NER_MODEL)
        print(f"[mem] after GLiNER load: {model_store.rss_mb()} MB", flush=True)
        return m

    return model_store.get_model("gliner", load)
